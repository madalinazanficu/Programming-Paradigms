...
(define (compute-length get-line-segment get-start-point get-end-point)
  (let* ([line (get-line-segment)]
         [start (get-start-point line)]
         [end (get-end-point line)]
         [x_start (car start)]
         [y_start (cdr start)]
         [x_end (car end)]
         [y_end (cdr end)])
    (sqrt (+ (sqr (- x_start x_end)) (sqr (- y_start y_end))))))
...
(define (compute-perimeter points)
 (let rec ([L (append points (list (car points)))] [P 0])
      (if (equal? (length L) 1)
          P
          (rec (cdr L) (+ (distance (car L) (cadr L)) P)))))         

...
(define (deep-rev L)
  (cond [(null? L) null]
        [(not (list? (car L)) ) (append (deep-rev (cdr L))  (list( car L)))] 
        [else (append (deep-rev (cdr L) ) (list (deep-rev (car L) ) ) )] ))
...
(define (palindrome? L)
  (define reversed_L (rev L))
  (if (equal? L reversed_L)
      #t
      #f)
  )
...
(define (palindrome-list? L)
  (define rev_l (rev (deep-rev  L)))
  (if (not (equal? L rev_l))
      #f
      #t))
...
(define (all-palindromes? n Bases)
  ;(define l_currentB (num->base n (car Bases))) ; lista ce contine reprezentarea in baza Bases[current_level] a numarului n
  ;(define rev_currentB (rev l_currentB))        ; lista inversa lisei l_currentB

  ;(cond [(null? Bases) #t]
        ;[(not(equal? l_currentB rev_currentB)) #f]
        ;[else (all-palindromes? n (cdr Bases))]))

  (cond [(null? Bases) #t]
        [(not(equal? (num->base n (car Bases)) (rev (num->base n (car Bases))))) #f]
        [else (all-palindromes? n (cdr Bases))]))
...
(define (palindromes-to-n n Bases)
  (if (< n 0)    ; oprire recursivitate => returnez o lista goala
      null
      (if (all-palindromes? n Bases)
          (append (palindromes-to-n (sub1 n) Bases) (list n))
          (append (palindromes-to-n (sub1 n) Bases) null))))
...
(define (first-b-pal start b)
  (define nr-bases (palindrom-min-bases start '(2 3 4 5 6 7 8 9 10)))
  (if (equal? nr-bases b)
      start
...
; Functie ce returneaza numarul de baze in care este un numar palindrom
(define (palindrom-min-bases n Bases)
  (if (empty? Bases)
      0
      ; Numarul n in baza curenta b (define n_b (num->base n (car Bases)))
      (if (palindrome? (num->base n (car Bases)))
           (add1 (palindrom-min-bases n (cdr Bases)))       ; adaug +1
           (palindrom-min-bases n (cdr Bases))))            ; adaug +0
...
; Transform numarul intr-o lista si apelez search
(define (longest-palindrome n)
  (define L (to-list n))
  (search-longest-palindrome L (length L)))

; Vreau o lungime de L, L-1, L-2, L-3  ... 0
(define (search-longest-palindrome L dim)
  (if (equal? dim 0)
      0
      (if (null? (check-palindrome L dim))
          (search-longest-palindrome L (sub1 dim))
          (list->num (check-palindrome L dim)))))

; Primeste o lista si lungimea pe care o cauta
(define (check-palindrome L k)
  (if (> k (length L))
      null
      (if (palindrome? (take L k))
          (take L k)
          (check-palindrome (cdr L) k))))

; Transform un numar intr-o lista
; Corner case daca numarul e zero nu va functiona
(define (to-list n)
  (if (equal? n 0)
      null
      (append (to-list(quotient n 10))(list(modulo n 10)))
  ))

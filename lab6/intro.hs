...
-- if-then-else
reverseList :: [a] -> [a]
reverseList l = if null l then 
                [] 
            else 
                (reverseList (tail l)) ++ [(head l)]
...
reverseList4 l 
    | null l = []
    | otherwise = reverseList3 (tail l) ++ [head l]

-- foldl - Parcurgere de la stanga la dreapta
-- 1 2 3 4 => [] => [1] => [2, 1] => [3, 2, 1] => [4, 3, 2, 1]
reverseList5 :: [a] -> [a]
reverseList5 l = foldl (\acc x -> x:acc) [] l


-- foldr - Parcurgere de la dreapta la stanga
reverseList6 :: [a] -> [a]
-- Varianta 1
reverseList6 l = reverse (foldr (\x acc -> x:acc) [] l)
-- Varianta 2
...
removeDuplicatesLeft l = reverse (foldl (\acc x -> if ((elem x acc) == False) then
                                                        x:acc
                                                    else
                                                        acc) [] l)
...
removeDuplicatesRight l = foldr (\x acc ->  if ((elem x acc) == False) then
                                                x:acc
                                            else
                                                acc) [] l
...
computeLength2 getLineSegment getStartPoint getEndPoint =
        ...
        x1 = (fst $ getStartPoint segment)
        x2 = (fst $ getEndPoint segment)
        y1 = (snd $ getStartPoint segment)
        y2 = (snd $ getEndPoint segment)
...
mergeSort :: [Int] -> [Int]
mergeSort l =
    let computeHalf f lst = f (quot (length lst) 2) lst
        fstHalf lst = (computeHalf take lst)
        sndHalf lst = (computeHalf drop lst)
    in
        case l of
        [] -> []
        [x] -> l
        _ -> (merge (mergeSort (fstHalf l)) (mergeSort (sndHalf l)))
...
mergeSort2 l = 
    case l of
        [] -> []
        [x] -> l
        _ -> (merge (mergeSort (fstHalf l)) (mergeSort (sndHalf l))) where
            computeHalf f lst = f (quot (length lst) 2) lst
            fstHalf lst = (computeHalf take lst)
            sndHalf lst = (computeHalf drop lst)

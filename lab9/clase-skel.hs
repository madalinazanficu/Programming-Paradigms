...
class (Num a) => MyClass a where
    funcAdd1 :: a -> a
    funcAdd2 :: a -> a

    funcAdd1 = (\x -> x - 1).funcAdd2
...
instance Eq a => Eq (BST a) where
    (==) BSTNil BSTNil = True
    (==) (BSTNod root1 left1 right1) (BSTNod root2 left2 right2) = (root1 == root2) && (left1 == left2) && (right1 == right2)
    (==) _ _ = False
...
printLevel :: Show a => Int -> BST a -> [Char]
printLevel  _ BSTNil = ""
printLevel  level (BSTNod root left right) = (replicate level '\t') ++ (show root) ++ "\n"
    ++ (printLevel (level + 1) left)
    ++ (printLevel (level + 1) right)

instance Show a => Show (BST a) where
    show tree = printLevel 0 tree

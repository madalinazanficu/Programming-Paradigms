...
% iau elementul maxim care este ultimul
selectFirst(List, Maxim) :- sort(List, List),
                            length(List, Len),
                            Len is Len - 1,
                            nth0(Len, List, Maxim)  ,!.

selectFirst(List, Maxim) :- nth0(0, List, Maxim).




% identify(+ListIn, -ListOut)
identify([], _).
identify([H | T], [H1 | T1]) :- selectFirst(H, Maxim),
                                H1 is (Maxim, H),
                                identify(T, T1).
...
mirror(L, Result) :- reverse(L, [_ | Lrev]), append(L, Lrev, Result).

fi(0, 0):- !.
%fi(N, Ans):- fi(N_prev, Ans_prev), N_prev is N -1, Ans is Ans_prev + N * N.
%fi(N, Ans):- N_prev is N - 1, fi(N_prev, Ans_prev), Ans is Ans_prev + N * N.
fi(N, Ans):- N_prev is N - 1, fi(N_prev, Ans_prev), Ans is Ans_prev + N * N.

face_AI(corina).
face_PP(vlad).

invata_prolog(X) :- face_PP(X), face_AI(X).
invata_prolog(X) :- face_PP(X).
invata_prolog(X) :- face_AI(X).


predicat(X, X, X).
predicat(X, Y, Z) :- X < Y, NewY is Y -X,
                     predicat(X, NewY, Z).
predicat(X, Y, Z) :- X > Y, NewX is X -Y,
                    predicat(NewX, Y, Z).


unitate_para(U) :- member(U, [0, 2, 4, 6, 8]).
unitate_impara(U) :- \+ unitate_para(U).            


myConcat([], L2, L2).
myConcat([H1 | Rest1], L2, [H1 | ListRez1]):- myConcat(Rest1, L2, ListRez1).

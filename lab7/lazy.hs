...
findStringsLongerThanTenChars l = filter  (\cuvant -> (length cuvant) > 9) l

findStringsLongerThanTenChars2 :: [String] -> [String]
findStringsLongerThanTenChars2 l = [x | x <- l, length x > 9]
...
buildPairsStringLength l = map (\cuvant -> (cuvant, length cuvant)) l
...
setDiff :: Eq a => [a] -> [a] -> [a]
setDiff a b = [x | x <- a, (elem x a) && (elem x b) == False]

cartProduct :: [a] -> [b] -> [(a, b)]
cartProduct a b = [(x, y) | x <- a, y <- b]

setUnion :: Eq a => [a] -> [a] -> [a]
setUnion a b = [x | x <- (setDiff a b) ++ (setDiff b a) ++ (setIntersection a b)]
...
newtonRaphson x g dg =
    let x_new = x - g(x) / dg(x) in
    [x] ++ (newtonRaphson x_new g dg)
...
{-
    ...
    dropWhile - primeste un predicat si o lista, si renunta la toate elementele care indeplinesc conditia
    zip - primeste flux1 si flux2 si construieste un flux de perechi (x, y), x aparinte flux 1 si y apartien flux2
 -}

newtonRaphsonSolve :: Double -> Double -> (Double -> Double) -> (Double ->Double) -> Double
newtonRaphsonSolve x_old atol g dg = 
    let flux1 = newtonRaphson x_old g dg
        flux2 = tail (newtonRaphson x_old g dg) in
    let fluxPairs = dropWhile (\(x, y) -> (abs(x - y) > atol)) (zip flux1 flux2) in
        snd (head fluxPairs)
...
babylonianMethod a x_old atol = 
    let f =  ( + (-a)).(** 2)
        df = ((2) *)  in
    newtonRaphsonSolve x_old atol f df

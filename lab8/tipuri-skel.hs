...
dotV v1@(V vx vy vz) v2@(V vx2 vy2 vz2) = vx * vx2 + vy * vy2 + vz * vz2
...
data BST a = BSTNode {
    val::a,
    myleft:: (BST a),
    myright:: (BST a) } | BSTNil deriving Show

insertElem :: (Ord a, Eq a) => BST a -> a -> BST a
insertElem BSTNil element = BSTNode element BSTNil BSTNil
insertElem nodu@(BSTNode value left right) element
  | value == element = nodu
  | value < element = BSTNode value left (insertElem right element)
  | value > element = BSTNode value (insertElem left element) right

findElem :: (Ord a, Eq a) => BST a -> a -> Maybe a
findElem BSTNil _ = Nothing
findElem nodu@(BSTNode value left right) element
  | value == element = Just value
  | element > value = findElem right element 
  | element < value = findElem left element

inorder :: BST a -> [a]
inorder BSTNil = []
inorder nodu@(BSTNode value left right) = [value] ++ (inorder left) ++ (inorder right)

size :: (BST a) -> Int
size BSTNil = 0
size nodu@(BSTNode value left right) = 1 + (size left) + (size right)

height :: (BST a) -> Int
height BSTNil = 0
height nodu@(BSTNode value left right) = 1 + max (height left) (height right)

isBalanced :: (Ord a, Num a) => BST a -> Bool
isBalanced BSTNil = True
isBalanced nodu@(BSTNode value left right) = if (abs((height left) - (height right)) <= 1) then
